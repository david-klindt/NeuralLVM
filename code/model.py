import torch
import numpy as np
from code.utils import get_grid, angle2vector, vector2angle, make_tuple
from code.hyperspherical_vae import reparameterize
...
class FeatureBasis(torch.nn.Module):  # one for each latent manifold
    def __init__(
        ...
        if feature_type == 'gauss':
            ...
            log_var_init = torch.log(torch.ones(var_shape) * np.pi**2)

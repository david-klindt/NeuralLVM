...
for imod, model_type in enumerate(['cosyne', 'orig']):
    ...
    for rep in range(reps):
        for iparam in range(len(Ntrains)):
            ...
            lat_err = eval_lats(z_pred_test[:, 0], z_test[:, 0], res = 200)
...
for i, dat in enumerate(dats):
    ...
    for imod in range(2):
        ...
        ax.errorbar(xs[i], m, yerr = s, color = cols[imod], ls = '-', capsize = 2)
...
plt.savefig('plots/mgplvm_result.png', bbox_inches = 'tight')
plt.close()